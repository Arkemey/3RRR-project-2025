...
h = 0.07 # taille coté triangle equi 5cm
...
def initialisation(angle_pos_ini):
    angle_pos_ini_deg = np.degrees(angle_pos_ini)
    tick = degres_to_moteur(angle_pos_ini_deg)
...
def inverse_kinematics(x,y):
    ...
    while(i < 3): #désormais on fait inverse kinematics sur chacun des trois points pour avoir nos 3 angles moteurs.
        ...
        print("angle alpha : %f " , angle_alpha[i] )
        print("angle beta : %f " , angle_beta[i] )
        print("xC, yC, %f , %f", xC[i], yC[i])
        i = i + 1
    return angle_alpha
...
def mouvement_vers(x,y):
    ...
    i = 0 
    while(i<3):
        tick_angleA = degres_to_moteur(angle_a[i])
        move_motor(DXL_IDS[i],tick_angleA[i])
        i = i + 1
...
def main(): 
    ...
    path = [] 
    while(i < 10):
        path.append((0,i*0.02)) # une ligne droite la du coup
        i = i+1
    
    initialisation(0.31655754) #valeur des angles pour etre au centre askip.
